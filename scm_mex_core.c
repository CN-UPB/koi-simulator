 ...
 */
void sumloop(int m, const double *exp_helper,const double *exp_t_coeff,
   const double *real_multiplier, const double *imag_multiplier, double t,
   ...
   m_i = 0;
   while (m_i < m) {
      angle = exp_helper[m_i] + exp_t_coeff[m_i]* t;
      complex_multiply(real_multiplier[m_i], imag_multiplier[m_i],
 ...
 * @param halfpi (pi/2)
 */
void sumloop_with_table(int m, const double *exp_helper,const double *exp_t_coeff,
   const double *real_multiplier, const double *imag_multiplier, double t,
 ...
 * @param *sq_Pn square root of Pn (size [k][n])
 ...
 */
int scm_sum(const long int sin_look_up_points, const int u, const int s, const int n, const int m, const int k, 
   ...
   const double *sq_Pn, int GainsAreScalar, double *re_h, double *im_h, double *output_SubPathPhase) {

   int i, t_i, n_i, m_i, u_i, s_i, k_i; /* running index variables */

   /* notation conversion variables from [][] to *(p[0]+var) */
   long int ksnm, kunm, knm, usntk, kn, kt, usn, usntk_i, ksn, kun;
   ...
   usn = u*s*n; /* for output pointing */
   ksn = k*s*n;  /* for parameter pointing */
   kun = k*u*n;
   one_over_sq_m = 1/sqrt(m);
            ...
            for(s_i=0;s_i< s; s_i++) {
               ...
               for(n_i=0;n_i<n;n_i++) {
                  ...
                  for (t_i=0; t_i<tn; t_i++) {
                     ...
                     imag_sum = 0;
                     kt = k*t_i + k_i;
                     sumloop_with_table(m, exp_helper, exp_t_coeff, real_multiplier, imag_multiplier,
                        *(ts+kt), &real_sum, &imag_sum, cos_table, r2p,divider, halfpi);
                     /* setting the output values and multiplying them with proper coefficient*/
                     usntk = usntk_i + usn*t_i;
                        *(re_h+usntk) = real_sum * *(sq_Pn+kn) * one_over_sq_m;
                        *(im_h+usntk) = imag_sum* *(sq_Pn+kn)* one_over_sq_m;
                  }
               }
            }
         }
            ...
            for(s_i=0;s_i< s; s_i++) {
               ...
               for(n_i=0;n_i<n;n_i++) {
                  ...
                  for (t_i=0; t_i<tn; t_i++) {
                     ...
                     sumloop(m, exp_helper, exp_t_coeff, real_multiplier, imag_multiplier,
                           *(ts+kt), &real_sum, &imag_sum);
 ...
 * @return 0 if success, 1 if bad arguments
 */
int scm_pol_sum(const long int sin_look_up_points, const int u, const int s, const int n, const int m, const int k,
   ...
   const int tn, const double *sq_Pn, int GainsAreScalar, int XpdIndependentPower,
   double *re_h, double *im_h, double *output_SubPathPhase) {

   int i, n_i, m_i, u_i, s_i, t_i, k_i;	/* running index variables */
   /* notation conversion variables from [][] to *(p[0]+var) */
   long int ksnm, kunm, knm, usntk, kn, kt, usn, usntk_i, ksn, kun;
   ...
   double halfpi;
   long int num_of_points, divider, pows_of_two;
   double one_over_sq_m, xpd_1, xpd_2, sq_r_n1, sq_r_n2;
 ...
 * prhs[17] = m, number of subpaths
 * prhs[18] = k, number of links
 * prhs[19] = tn, number of time samples
 * prhs[20] = GainsAreScalar, nonzero if gains are scalar
 ...
 * prhs[19] = [sq_Pn], square root of Pn (size [k][n])
 ...
 * prhs[24] = m, number of subpaths
 * prhs[25] = k, number of links
 * prhs[26] = tn, number of time samples
 * prhs[27] = GainsAreScalar, nonzero if gains are scalar
 * prhs[28] = XpdIndependentPower, nonzero if used
 * Output arguments are 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
   ...
   double *ts;
   int u, s, n, m, tn, k; /* parameters to be extracted from inputs' dimensions */
   ...
   if (mode == GENERAL) {
      ...
      GainsAreScalar = (int)mxGetScalar(prhs[20]);
      ...
      error = scm_sum(look_up_points, u, s, n, m, k, re_G_BS, im_G_BS, re_G_MS, im_G_MS,
         k_CONST, d_s, d_u, aod, aoa, phase, v, theta_v, ts, tn, sq_Pn, GainsAreScalar,
   ...
   /* check for polarized mode */
      ...
      if(nrhs != 29)
         mexErrMsgTxt("scmmex error: exactly 29 input arguments required");
      ...
      m = (int)mxGetScalar(prhs[24]);
      k = (int)mxGetScalar(prhs[25]);
      tn = (int)mxGetScalar(prhs[26]);
      GainsAreScalar = (int)mxGetScalar(prhs[27]);
      XpdIndependentPower = (int)mxGetScalar(prhs[28]);
      ...
      /* calculating the values */
      error = scm_pol_sum(look_up_points, u, s, n, m, k, re_X_BS_v, im_X_BS_v, re_X_BS_h, im_X_BS_h, 
         re_X_MS_v, im_X_MS_v, re_X_MS_h, im_X_MS_h, k_CONST, d_s, d_u, aod, aoa,
         phase_v_v, phase_v_h, phase_h_v, phase_h_h,	r_n1, r_n2,
         v, theta_v, ts, tn, sq_Pn, GainsAreScalar, XpdIndependentPower, re_h, im_h, output_SubPathPhase);
   } /* end polarized mode */
