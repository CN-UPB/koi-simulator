...
for m = 1:nCluster
    switch cluster(m).type
        case 1 %Single cluster
            ...
            fprintf(fh, '%0.16f ', 0.0);
            fprintf(fh, '%0.16f ', pos_VR);
            fprintf(fh, '%0.16f ', pos_BS);
            fprintf(fh, '%0.16f ', cluster(m).pos_c_BS);
            fprintf(fh, '%0.16f ', cluster(m).pos_c_MS);
            fprintf(fh, '%0.16f ', cluster(m).a_c_BS);
            fprintf(fh, '%0.16f ', cluster(m).b_c_BS);
            fprintf(fh, '%0.16f ', cluster(m).h_c_BS);
            fprintf(fh, '%0.16f ', cluster(m).shadow_f);
            fprintf(fh, '%0.16f ', cluster(m).tau_c_link);
