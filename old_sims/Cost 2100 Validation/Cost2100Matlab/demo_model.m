...
close all
rng(657845);
delete('../Cost2100C++/mpc.txt');
network = 'SemiUrban_300MHz'   %{'IndoorHall_5GHz', 'SemiUrban_300MHz'}
Band = 'Wideband'              %{'Wideband','Narrowband'}
Link = 'Single'              %{'Single', 'Multiple'}
Antenna = 'MIMO_omni'      %{'SISO_omni', 'MIMO_omni', 'MIMO_dipole', 'MIMO_measured'}
            ...
            case 'MIMO_omni'                
                ...
                switch Band
                    case 'Wideband'
                        ...
                        delete('../Results_Validation/MIMO_Matrix_matlab.txt');
                        diary('../Results_Validation/MIMO_Matrix_matlab.txt');
                        format longG;
                        Nfreq = freq(1):delta_f:freq(2);
                        for j=1:length(Nfreq)
                        if(real(H_omni_MIMO(1,j,1,1)) > 0)
                            fprintf('   %0.16f',real(H_omni_MIMO(1,j,1,1)));
                        else
                            fprintf('  %0.16f',real(H_omni_MIMO(1,j,1,1)));
                        end
                        
                        if(imag(H_omni_MIMO(1,j,1,1)) > 0)
                            fprintf('+%0.16fi',imag(H_omni_MIMO(1,j,1,1)));
                        else
                            fprintf('%0.16fi',imag(H_omni_MIMO(1,j,1,1)));
                        end
                        
                        if(real(H_omni_MIMO(1,j,1,2)) > 0)
                            fprintf('   %0.16f',real(H_omni_MIMO(1,j,1,2)));
                        else
                            fprintf('  %0.16f',real(H_omni_MIMO(1,j,1,2)));
                        end
                        
                        if(imag(H_omni_MIMO(1,j,1,2)) > 0)
                            fprintf('+%0.16fi \n',imag(H_omni_MIMO(1,j,1,2)));
                        else
                            fprintf('%0.16fi \n',imag(H_omni_MIMO(1,j,1,2)));
                        end
                        
                        if(real(H_omni_MIMO(1,j,2,1)) > 0)
                            fprintf('   %0.16f',real(H_omni_MIMO(1,j,2,1)));
                        else
                            fprintf('  %0.16f',real(H_omni_MIMO(1,j,2,1)));
                        end
                        
                        if(imag(H_omni_MIMO(1,j,2,1)) > 0)
                            fprintf('+%0.16fi',imag(H_omni_MIMO(1,j,2,1)));
                        else
                            fprintf('%0.16fi',imag(H_omni_MIMO(1,j,2,1)));
                        end
                        
                        if(real(H_omni_MIMO(1,j,2,2)) > 0)
                            fprintf('   %0.16f',real(H_omni_MIMO(1,j,2,2)));
                        else
                            fprintf('  %0.16f',real(H_omni_MIMO(1,j,2,2)));
                        end
                        
                        if(imag(H_omni_MIMO(1,j,2,2)) > 0)
                            fprintf('+%0.16fi \n\n',imag(H_omni_MIMO(1,j,2,2)));
                        else
                            fprintf('%0.16fi \n\n',imag(H_omni_MIMO(1,j,2,2)));
                        end
                        end
