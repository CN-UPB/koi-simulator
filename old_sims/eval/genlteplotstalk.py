...
def buildBarPlot(title, yaxis, data, mapFunction = lambda x: x, offsetx = 0, xlim=None, ylim=None,filename=None, drawTicks=True, drawLegend=True, numberOffset=0.5):
    ...
    lgd = None
    if(drawLegend):
        ax.legend([plt.Rectangle((0, 0), 1, 1, fc=COLORS[i]) for i in range(len(data[0][1]))], [elem[0] for elem in data[0][1]], loc="lower right")
...
buildBarPlot("Speedup of different approaches on one 12 core machine", "Speedup",
    ...
    ], mapFunction=lambda ele:(float(ele[0]["runtime"])/float(NUMBER_OF_TTIS))/(float(ele[1]["runtime"])/float(NUMBER_OF_TTIS)), offsetx=0.3, filename="lte-eval-1machine-talk.pdf", numberOffset=0.26)
...
buildBarPlot("Speedup of different approaches on four 12 core machines", "Speedup",
    ...
    ], mapFunction=lambda ele:(float(ele[0]["runtime"])/float(NUMBER_OF_TTIS))/(float(ele[1]["runtime"])/float(NUMBER_OF_TTIS)), offsetx=0.9, filename="lte-eval-4machines-talk.pdf", numberOffset=0.63, ylim=[0,48])
