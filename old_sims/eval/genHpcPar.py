...
for i in range(0, args.n):
    ...
    f.write("do\n");
    f.write("  tmp=\"-n 1 ./lteWrapper.sh " + args.c + args.suffix + " " + str(i) + " $x -c " + args.c + " --use-thread-pool=false --parallel-simulation=true --parsim-synchronization-class=" + args.sync + "\"\n");
    f.write("  if [ \"$x\" -ne 0 ];\n");
    f.write("   then\n");
    f.write("   cmd=\"$cmd : $tmp\"\n");
    f.write("   else\n");
    ...
    f.write("echo $cmd\n");
    f.write("echo \"-------------\"\n");
    f.write("$MPIEXEC -perhost 12 $MYFLAGS $cmd\n");
