...
void roundRobinScheduler::init(cModule *module){
...
void roundRobinScheduler::setSINR(mat sinr){
	// Round Robin Scheduler needs no SINR values.
}

Schedule* roundRobinScheduler::calcUpSchedule(cQueue *packetQueue, vector<int> *transmitRequests){
	...
	for(int i = 0; i < numberOfRessourceBlocks; ++i)  {
		...
		// ROUND ROBIN
		std::cout << "Up Schedule Computation (Round Robin): " << std::endl;
		//find a ms for the slot from the packet queues
		int fromMsId = sendingMs.at(i % numberOfSendingMs);
		...
		if(transmitRequests->at(fromMsId) == 0)  {
			sendingMs.erase(sendingMs.begin() + (i % numberOfSendingMs));
			numberOfSendingMs--;
		}
	}
	return schedule;
...
std::vector<int> roundRobinScheduler::calcDownSchedule(cQueue *packetQueue){
	vector<int> packetCount(numberOfMobileStations);
	...
	for(int i = 0; i < numberOfRessourceBlocks; ++i)  {
		...
		// ROUND ROBIN
		std::cout << "Down Schedule Computation (Round Robin): " << std::endl;
		int toMsId = sendingApps.at(i % numberOfSendingApps);
		...
		if(packetCount.at(toMsId) == 0)  {
			sendingApps.erase(sendingApps.begin()+(i % numberOfSendingApps));
			numberOfSendingApps--;
		}
	}
	return downSchedule;
