...
if length(active_VR)>0 %If there are active far cluster
    ...
    for m = 1:length(active_c)
        ...
        disp(a_c);
        disp(cluster(active_c(m)).shadow_f);
        ...
        for n = 1:paraSt.n_mpc
            ...
            disp(mpc(active_c(m)).a_mpc(n));
            
            diary('Results_Matlab_Channel_Info.txt');
            format long;
            disp([phi_mpc_BS theta_mpc_BS phi_mpc_MS theta_mpc_MS tau_mpc real(amp_h) imag(amp_h)]);
